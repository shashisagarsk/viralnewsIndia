...
export default function ArticleDetailClient({
  ...
  const { language, translateNews, formatShortDate, translateCategory, t } = useLanguage();
  const [copied, setCopied] = useState(false);

  const article = translateNews(initialArticle);
    ...
    <>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        ...
        <nav className="mb-6 flex items-center gap-2 text-xs text-gray-500 font-medium">
          ...
          <Link
            href={`/category/${initialArticle.category.toLowerCase()}`}
            className="hover:text-red-600 transition uppercase font-semibold"
          >
            {translateCategory(initialArticle.category)}
          </Link>
          <ChevronRight size={12} />
        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_320px]">
          {/* Main Article Column */}
          <article>
            {/* Top Bar: Category Pill, Breaking Badge & Quick Language Mode Switcher */}
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <Link
                  href={`/category/${initialArticle.category.toLowerCase()}`}
                  className="inline-block bg-red-600 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-white transition hover:bg-red-700 rounded-sm shadow-sm"
                >
                  {article.category}
                </Link>

                {article.breaking && (
                  <span className="inline-flex items-center gap-1 bg-red-100 px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wide text-red-700 rounded-sm">
              ...
              {/* Quick Article Translate Pill */}
              <div className="flex items-center gap-2">
                <span className="hidden sm:inline-flex text-[11px] font-bold text-gray-500 items-center gap-1">
                  <Languages size={13} className="text-red-600" />
                  <span>{t("translateMode")}:</span>
                </span>
                <LanguageSwitcher variant="article" />
              </div>
            </div>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-y border-gray-200 py-3.5 text-xs text-gray-500">
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={handleShare}
                  className="flex items-center gap-1.5 rounded-full border border-gray-200 px-3 py-1 text-[11px] font-semibold text-gray-700 transition hover:border-red-600 hover:text-red-600"
                  title={t("shareArticle")}
                >
                  {copied ? (
                    <>
                      <Check size={12} className="text-emerald-600" />
                      <span className="text-emerald-600 font-bold">{t("copiedLink")}</span>
                    </>
                  ) : (
                    <>
                      <Share2 size={12} />
            ...
            <div className="relative mt-8 aspect-[16/10] w-full overflow-hidden rounded-xl bg-gray-100 sm:aspect-[16/9] shadow-md">
              <Image
                src={article.image}
                alt={article.title}
                fill
                ...
                article.content.split("\n\n").map((block, idx) => {
                  ...
                  if (block.startsWith("> ")) {
                    return (
                      <blockquote
                        key={idx}
                        className="my-6 border-l-4 border-red-600 bg-red-50/50 p-5 font-serif italic text-gray-800 text-lg leading-snug rounded-r-lg"
                      >
                        {block.replace("> ", "")}
                      </blockquote>
                    );
                  ...
                  return (
                    <p key={idx} className="text-gray-800 leading-8 text-[15px] sm:text-base">
                      {block}
                    </p>
                  );
                })
            ...
            <div className="mt-10 flex items-center gap-4 rounded-xl border border-gray-200 bg-gray-50 p-6">
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-red-600 font-black text-xl text-white shadow">
                {article.author.charAt(0)}
              </div>
              <div>
                <h4 className="text-sm font-black text-gray-950">
                  {t("reportedBy")} {article.author}
                </h4>
            ...
            <section className="mt-14 border-t-2 border-gray-950 pt-8">
              <h3 className="text-lg font-black uppercase tracking-wider text-gray-950 mb-6">
                {t("relatedStoriesIn")} {translateCategory(initialArticle.category)}
              </h3>
